...
**The files are numbered `01` through `08` in exactly the order you should read them — so just go top to bottom.** This document explains *why* that order, and what to cut if you're short on time.

The one thing not to do: **don't jump to the most impressive-sounding one.** Each document assumes you already have specific vocabulary from the ones before it. Read out of order and you'll hit answers that casually say "the cross-encoder reranks the RRF-fused candidates" before anything has told you what any of those three words mean.
