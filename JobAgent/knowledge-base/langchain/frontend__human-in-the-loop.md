...
When the agent pauses, `stream.interrupt` contains a [HITLRequest](https://reference.langchain.com/python/langchain/agents/middleware/human_in_the_loop/HITLRequest) with the following structure:
...
and continues execution. Each entry in
...
- **Persist the interrupt state**. If the user refreshes the page, the interrupt should still be visible. [`useStream`](https://reference.langchain.com/javascript/langchain-react/index/useStream) handles this via the thread’s checkpoint.
