...
Mapping of tool names to approval configs. Values can be `` (interrupt with default config), `` (auto-approve), or an `` object.
...
Optional predicate that receives a [ToolCallRequest](https://reference.langchain.com/python/langgraph.prebuilt/tool_node/ToolCallRequest) and returns `` to interrupt or `` to auto-approve. Use it to gate interrupts on a call’s arguments. Requires ``.
