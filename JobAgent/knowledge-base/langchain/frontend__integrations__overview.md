...
[CopilotKitFull AI chat runtime with structured generative UI support. Add a custom CopilotKit endpoint to your LangGraph deployment, then render dynamic component trees in React.](/oss/python/langchain/frontend/integrations/copilotkit)

[AI ElementsComposable shadcn/ui-based components for AI chat. Drop in `Conversation`, `Message`, `Tool`, and `Reasoning` and wire them directly to `stream.messages`.](/oss/python/langchain/frontend/integrations/ai-elements)

[assistant-uiHeadless React framework with a full runtime layer. Bridge `useStream` to `AssistantRuntimeProvider` via the `useExternalStoreRuntime` adapter.](/oss/python/langchain/frontend/integrations/assistant-ui)

[OpenUIGenerative UI library that lets the agent produce complete, interactive dashboards in a declarative component DSL. Purpose-built for data-rich, report-style UIs.](/oss/python/langchain/frontend/integrations/openui)
